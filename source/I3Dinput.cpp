...
I3Dinput::I3Dinput(I3Deditor *in_parent, EditorWindow *in_win): _i3d(in_parent), _win(in_win) {
  _ix= &in_win->ix;
  scene= null;
  meshSel= matSel= 0;
}
...
bool I3Dinput::loadFile() {
  ...
  */
  ...
  //ofn.hInstance=    (HINSTANCE)_ix->win->_hInstance;
...
bool I3Dinput::loadFileName(cchar *in_name, bool useIxConsole) {
  ...
  // aiProcess_ImproveCacheLocality !! this will process and rearange the vertices !!! this can be huge improvement for a big mesh <<<<<<<<<<<<<<<<
  // aiProcess_CalcTangentSpace
  // aiProcess_GenSmoothNormals
  // aiProcess_Triangulate 
  // aiProcess_PopulateArmatureData - this has to happen for IX
  // aiProcess_LimitBoneWeights this forces 4 bones max, also important for IX
  scene= aiImp.ReadFile(in_name, aiProcess_PopulateArmatureData| aiProcess_LimitBoneWeights);
  if(scene== null) {
    if(useIxConsole)
