...
TEXui::TEXui(TEXeditor *in_parent, EditorWindow *in_win): _tex(in_parent), _win(in_win) {
  _ix= &in_win->ix;
  w1= w2= null;
...
void TEXui::init() {
  ...
  // ALSO, WINDOWS STYLE MUST BE CENTRALIZED; <<<<<<<<<<<<<<<<<<
  ...
  */
  ...
  w2= _ix->wsys().createWindow("TEX",        null, _ix->win->x0+ _ix->win->dx- width, _ix->win->y0, width, _ix->win->dy);
  ...
  // mesh --------------------
  p.y+= 15;
  i.lMesh= _ix->wsys().createDropList(w1, p.x, p.y, w1->pos.dx- 6- 15, 15);
  ...
  p.y+= 30;
  i.lMats= _ix->wsys().createDropList(w1, p.x, p.y, w1->pos.dx- 6- 15, 15);
...
/*
void TEXui::updateInputMeshDetails() {
  str8 s;
  if(_i3d->input.scene) {
...
/*
void TEXui::updateInputMatDetails() {
  str8 s;
  if(_i3d->input.scene) {
...
/*
void TEXui::updateOutputMeshDetails() {
  str8 s;
  ...
  o.dataType->select(_i3d->output.outMesh.dataType);
...
/*
void TEXui::updateI3OfromControls() {
...
void TEXui::update() {
  ...
  if(osi.flags.windowResized) {
    w1->resize(w1->pos.dx, _ix->win->dy);
    w2->resize(w2->pos.dx, _ix->win->dy);
  }
  ...
  if(_win->wsysUpdate()) {
    ...
    if(o.importFromCurrent->is.activated) {
      ...
      _i3d->output.inportMesh_n(&_i3d->output.outMesh, _i3d->input.scene, _i3d->input.meshSel);
